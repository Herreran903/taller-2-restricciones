...
Se experimentó con distintas \textbf{estrategias de búsqueda}, variando tanto la heurística de selección de variables como la de asignación de valores. Las estrategias consideradas fueron:
\begin{itemize}
    \item \texttt{first\_fail + indomain\_min}: selecciona la variable con el dominio más pequeño y le asigna el valor mínimo posible.
    \item \texttt{dom\_w\_deg + indomain\_split}: prioriza variables con alto peso de restricciones insatisfechas (mayor \textit{grado ponderado}) y divide su dominio, aprovechando información sobre conflictos acumulados.
    \item \texttt{input\_order + indomain\_min}: asigna variables en el orden en que fueron declaradas, con el valor mínimo.
\end{itemize}